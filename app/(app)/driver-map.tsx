...
import {
  areAllPointsInOtherList,
  getDistanceFromLatLonInKm,
} from "@/utilities";
import { useAtom, useAtomValue, useSetAtom } from "jotai";
import {
  directionCoordinatesAtom,
  directionPolylineAtom,
  fullBinsCoordinatesAtom,
} from "@/atoms";
...
export default function MapScreen() {
  ...
  const directionPolyline = useAtomValue(directionPolylineAtom);
  const directionCoordinates = useAtomValue(directionCoordinatesAtom);

  const fullBinsRef = useRef<MarkerData[]>([]);

  const canShowDirections = isShowingRoutes && directionPolyline.length > 0;

  const markedLegs =
    directionCoordinates.state === "hasData"
      ? directionCoordinates.data.trips[0].legs.map((leg, index) => ({
          ...leg,
          index,
        }))
      : [];
  ...
  useEffect(() => {
    try {
      ...
      const unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        const fullBins: MarkerData[] = fetchedMarkers.filter(
          (marker) => marker.status === MarkerStatus.Full
        );
        if (fullBinsRef.current) {
          if (!areAllPointsInOtherList(fullBins, fullBinsRef.current)) {
            fullBinsRef.current = fullBins;
            if (location) {
              setFullBinsCoordinates([
                {
                  longitude: location.coords.longitude,
                  latitude: location.coords.latitude,
                },
                ...fullBins.map((marker) => marker.coordinate),
              ]);
            }
          }
        }
  ...
  const cleanBin = async () => {
    ...
    const index = markers.findIndex(
      (marker) => marker.id === (closestBin as MarkerData).id
    );
    markers[index] = {
      ...markers[index],
      status: MarkerStatus.Empty,
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {canShowDirections &&
          directionCoordinates.state === "hasData" &&
          directionCoordinates.data.waypoints.map((waypoint, index) => (
            <Marker
              key={waypoint.hint}
              coordinate={{
      ...
      <TouchableOpacity
        ...
        onPress={() => setIsShowingRoutes((isShowingRoute) => !isShowingRoute)}
      >
            ...
            <View style={styles.popupContainer}>
              ...
              <View style={styles.binOptions}>
                <Text style={styles.deletePromptText}>
                  θέλεις να διαγράψεις σίγουρα τον κάδο;
                </Text>
                <TouchableOpacity
                  onPress={deleteBin}
